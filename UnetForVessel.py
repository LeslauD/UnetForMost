...
batch_size = 4
seed = 2019
image_genV = imgen().flow(Raw_prep(Train_Img),shuffle=False, batch_size=batch_size, seed=seed)
mask_genV = imgen().flow(Mask_prep(Train_VMask),shuffle=False, batch_size=batch_size, seed=seed)
GeneratorV = zip(image_genV, mask_genV)
valdataV = (Raw_prep(Val_Img), Mask_prep(Val_VMask))
...
model_checkpoint = ModelCheckpoint('../result/1D/vessel/modelV.hdf5', monitor='val_loss',verbose=1, save_best_only=True)
...
historyV = modelV.fit_generator(GeneratorV, validation_data=valdataV, epochs=epoch, verbose=1, 
...
def saveModelSummary(model):
    f=open('../result/1D/vessel/modelSummary.txt','w')
...
lastepoch=lastEpoch(historyV,time_callback,epoch)
...
Fit_plot(historyV, batch_size, epoch,lastepoch,'modelV') 


# In[51]:


PredV = predict(Test_Img,modelV)


# In[52]:


print('Dice of test data:',dice_coef(Test_VMask, PredV))
dice_plot(Test_VMask, PredV,'modelV')
pred_plot(Test_Img, Test_VMask, PredV, 'modelV')
